    ...
    private func updateSubViewFrame() {
        ...
        for (index, view) in subviews.enumerated() {
            ...
            if cornRadius>0{
                if index == 0{
                    ///有圆角 第一个 设置左边
                    view.SetMutiBorderRoundingCorners(byRoundingCorners: [.topLeft,.bottomLeft], corner: cornRadius)
                }else if index==(subviews.count-1){
                    ///最后一个 设置右边的圆角
                    view.SetMutiBorderRoundingCorners(byRoundingCorners: [.topRight,.bottomRight], corner: cornRadius)
                }
            }
        }
        ...
        if currentPage >= 0 && currentPage < subviews.count {
            ...
            if cornRadius>0{
                if currentPage == 0{
                    ///有圆角 第一个 设置左边
                    currentView.SetMutiBorderRoundingCorners(byRoundingCorners: [.topLeft,.bottomLeft], corner: cornRadius)
                }else if currentPage==(subviews.count-1){
                    ///最后一个 设置右边的圆角
                    currentView.SetMutiBorderRoundingCorners(byRoundingCorners: [.topRight,.bottomRight], corner: cornRadius)
                }
            }
        }
    }
